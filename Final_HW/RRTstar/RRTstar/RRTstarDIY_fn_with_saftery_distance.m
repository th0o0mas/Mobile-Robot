function RRTstarDIY_fn(app)
    ...
    inflate(InflatedMap, safe_distance);
    sv.Map = InflatedMap;
    ...
    % ==========================================================
    % DIAGNOSTIC CHECK: START/END VALIDITY AFTER INFLATION
    % ==========================================================
    
    % The first state for isStateValid needs to be the position followed by an angle (e.g., pi)
    % The 'sv' object is the validator we created earlier.
    
    start_state_check = [startpos' pi];
    end_state_check = [endpos' pi];
    
    % Check if the starting position is valid on the inflated map
    start_valid = isStateValid(sv, start_state_check); 
    
    if ~start_valid
        error('CRITICAL ERROR: Start position is INVALID on the Inflated Map. The inflation radius (safe_distance) is likely too large, or startpos is too close to an obstacle. Reduce safe_distance!');
    end
    
    % Check if the ending position is valid on the inflated map
    end_valid = isStateValid(sv, end_state_check);
    
    if ~end_valid
        error('CRITICAL ERROR: End position is INVALID on the Inflated Map. The goal region is too close to an obstacle. Reduce safe_distance!');
    end
    
    disp(['Start and End positions are validated on the inflated map. RRT* planning can proceed...']);
    ...
    % Spatial indexing cache for faster nearest neighbor (Not fully implemented/used in original logic)
    ...
    while i <= iter
        ...
        while ~sampling_success && sample_attempts < max_sample_attempts
            ...
            % Validity check
            theta = 0;  % or rand()*2*pi - pi
            isValid = isStateValid(sv, [pos_rand' theta]);
            if isValid
        ...
        root.BFSt(); % Ensure tree is consistent after additions/changes
        ...
        if dist_to_goal <= tolerance
            % Add the final goal node connection
            endpos_node = X_new.addChild(endpos);
            
            % Track if we improved
            if endpos_node.cost < best_cost
                best_cost = endpos_node.cost;
        ...
        if i == iter
            if solution_found && ~isempty(best_path_node)
                ...
                for j = 1:numel(z)
                    ...
                    % Logic seems to prefer NearNeighbor within maxconnection radius
                    % The second condition here looks like an alternative if
                    % NearNeighbor is not found, but it's confusingly written.
